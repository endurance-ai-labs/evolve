...
const ret = (n, dec = 2, suffix = "%") => {
  ...
  const sign = n > 0.0001 ? "+" : "";
  return `<span class="ev-ret ${c}">${sign}${Number(n).toFixed(dec)}${suffix}</span>`;
};
const money = (n, dec = 0) => {
  const c = n > 0 ? "up" : n < 0 ? "dn" : "flat";
  return `<span class="ev-ret ${c}">${n > 0 ? "+" : ""}${fmt$(n, dec)}</span>`;
};
...
/* =========================================================
   PERSONAS — the Evolve org drives access
   Permission flags:
   ...
   ========================================================= */
const PERSONAS = [
  { id: "ceo",    name: "EJ Kahn",              title: "Founder & Managing Partner",
    init: "EK", perms: { firm: 1, revenue: 1, book: 1, research: 1, trading: 1, compliance: 1 }, access: "Full firm access" },
  { id: "cio",    name: "Razmig Der-Tavitian",  title: "Chief Investment Officer & Managing Partner",
    init: "RD", perms: { firm: 1, revenue: 0, book: 1, research: 1, trading: 1, compliance: 0 }, access: "Investment team" },
  { id: "research", name: "Glen Strauss",       title: "Managing Partner \u2014 Alternatives",
    init: "GS", perms: { firm: 1, revenue: 0, book: 1, research: 1, trading: 0, compliance: 0 }, access: "Research & due diligence" },
  { id: "pm",     name: "Hank Krakover",        title: "Portfolio Manager",
    init: "HK", perms: { firm: 1, revenue: 0, book: 1, research: 1, trading: 1, compliance: 0 }, access: "Portfolio management" },
  { id: "advisor1", name: "Jonathan Lewis",     title: "Managing Partner \u2014 New York",
    init: "JL", perms: { firm: 0, revenue: 0, book: 0, research: 1, trading: 0, compliance: 0 }, access: "Advisory \u2014 own book" },
  { id: "advisor2", name: "Carly Whitmore",     title: "Wealth Adviser",
    init: "CW", perms: { firm: 0, revenue: 0, book: 0, research: 1, trading: 0, compliance: 0 }, access: "Advisory \u2014 own book" },
...
const ADVISOR_OF = { advisor1: "Jonathan Lewis", advisor2: "Carly Whitmore" };
/* The client service associate supports these advisers. */
const CSA_SUPPORTS = ["Jonathan Lewis", "Carly Whitmore"];
/* The external persona is scoped to one household. */
const CLIENT_HH = "HH-0001";

function currentRole()    { try { return localStorage.getItem("ev-role") || ""; } catch (e) { return ""; } }
...
function setRole(id) {
  try {
    localStorage.setItem("ev-role", id);
    const p = PERSONAS.find((x) => x.id === id);
    localStorage.setItem("ev-mode", p && p.perms.external ? "client" : "internal");
  } catch (e) {}
  window.location.reload();
}
function signOutUser() { try { localStorage.removeItem("ev-role"); } catch (e) {} window.location.reload(); }
...
function gatePassed() { try { return localStorage.getItem("ev-gate") === "1"; } catch (e) { return true; } }
function submitGate(e) {
  e.preventDefault();
  const i = document.getElementById("gate-input");
  if (i.value.trim().toLowerCase() === GATE_PHRASE) {
    try { localStorage.setItem("ev-gate", "1"); } catch (err) {}
    window.location.reload();
  } else {
  ...
  document.body.insertAdjacentHTML("beforeend", `
    <div class="login-overlay">
      <div class="login-box" style="max-width:420px">
        ...
        <div class="login-title">Evolve Private Wealth</div>
        ...
        <div class="login-foot" style="margin-top:18px">
          Evolve Private Wealth is a fictional firm and all data is synthetic.
          <a href="/evolve/welcome/">About the platform &rarr;</a>
        </div>
      </div>
    </div>`);
    ...
    <div class="login-overlay">
      <div class="login-box">
        ...
        <div class="login-foot">
          Demonstration environment. Evolve Private Wealth is a fictional firm and every household, holding,
          fund, figure and document in this portal is synthetic.
          <a href="/evolve/welcome/">Read about the platform &rarr;</a>
        </div>
      </div>
    </div>`);
}
...
const SRC = {
  ...
  brain: ["Evolve Brain", "Generated from the live portfolio data"],
};
...
function apprState(key, n) {
  try {
    const raw = JSON.parse(localStorage.getItem("ev-appr:" + key) || "[]");
    return Array.from({ length: n }, (_, i) => !!raw[i]);
  } catch (e) { return Array.from({ length: n }, () => false); }
}
function apprSave(key, arr) {
  try { localStorage.setItem("ev-appr:" + key, JSON.stringify(arr)); } catch (e) {}
}
function apprComplete(key, n) { return apprState(key, n).every(Boolean); }
function resetChain(key) { try { localStorage.removeItem("ev-appr:" + key); } catch (e) {} window.location.reload(); }
...
function slackNudge(key, idx) {
  ...
  ov.innerHTML = `
    <div class="slk-card">
      <div class="slk-head"><span class="slk-x" onclick="this.closest('.slk-ov').remove()">✕</span>
        <b>Evolve Private Wealth</b><span class="slk-dm-label">Direct message</span></div>
      <div class="slk-peer">
  ...
  setTimeout(() => {
    ...
    try { localStorage.setItem(`ev-nudge:${key}:${idx}`, stamp); } catch (e) {}
  }, 1900);
}
...
function sigBlock(key, roleId, caption) {
  let signed = "";
  try { signed = localStorage.getItem("ev-sig:" + key) || ""; } catch (e) {}
...
function signDoc(key, roleId) {
  ...
  try { localStorage.setItem("ev-sig:" + key, `${persona ? persona.name : roleId}|${when}`); } catch (e) {}
  window.location.reload();
}
...
function disclosure(extra) {
  return `<div class="rp-disc">
    <b>Demonstration environment.</b> Evolve Private Wealth, LLC is a fictional firm. Every household, account,
...
function gate(title, msg) {
  return `<div class="ev-gate"><b>${esc(title)}</b>${esc(msg)}</div>`;
}
...
function boot(opts, fn) {
  ...
  try {
    ...
    app.innerHTML = `<div class="ev-gate"><b>This page could not be rendered</b>
