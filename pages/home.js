...
if (!isSignedIn() && location.search.indexOf("enter") < 0) {
  location.replace("/evolve/welcome/");
}
...
function openItemsPanel() {
  ...
  drifted.slice(0, 4).forEach((h) => {
    ...
    items.push(["warn", h.name + " is out of tolerance",
      ...
      "/evolve/trading/?hh=" + h.id, "Open the rebalance proposal"]);
  });
  ...
  overdueIps.slice(0, 2).forEach((h) => {
    items.push(["", "Investment policy review past due: " + h.name,
      ...
      "/evolve/households/household/?id=" + h.id + "&tab=ips", "Review the policy"]);
  });
    ...
    <div class="rp-scroll" style="max-height:340px">
      <table class="demo-tbl">
        ...
        <tbody>${top.map((h) => {
          ...
          return `<tr class="rp-click" onclick="location.href='/evolve/households/household/?id=${h.id}'">
...
function marketPanel() {
  ...
  return panel("Markets", `
    ...
    <div class="rp-note" style="margin-top:12px">
      ...
      <a href="/evolve/markets/" style="color:var(--color-blue)">Open the full market desk &rarr;</a>
    </div>`,
  ...
  <div class="demo-grid demo-two" style="margin-top:22px">
    ${panel("Your goals", goals.length ? `
      <table class="demo-tbl" style="width:100%">
        ...
        <tbody>${goals.map((g) => `<tr>
          ...
          <td class="num"><b class="ev-ret ${g.funded >= 1 ? "up" : "dn"}">${Math.round(g.funded * 100)}%</b></td>
        </tr>`).join("")}</tbody>
      </table>
    ...
    ${panel("Recent documents", `
      ...
      <div class="rp-note" style="margin-top:6px">
        <a href="/evolve/documents/" style="color:var(--color-blue)">Open your document vault &rarr;</a></div>`,
      { k: "Your vault" })}
  </div>
