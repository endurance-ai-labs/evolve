    ...
    ${panel("What is happening here", `
      ...
      <dl class="rp-dl">
        ...
        <dt>Next review due</dt><dd>${h.ipsReview < RP.asOf
          ? '<span class="ev-ret dn">' + fmtDate(h.ipsReview) + " &middot; past due</span>"
...
function hhWorld(h, w) {
  const sheet = [
    ["Managed by Evolve", h.mv, "Custodied and discretionary"],
    ...
    <div class="rp-scroll">
      <table class="demo-tbl">
        ...
        <tbody>${pos.map((p) => {
          ...
          return `<tr class="rp-click" onclick="location.href='/evolve/funds/fund/?id=${p.fundId}'">
  ...
  ${panel("Commitments", `
    <div class="rp-scroll">
      <table class="demo-tbl">
        ...
        <tbody>${cs.map((c) => `<tr class="rp-click" onclick="location.href='/evolve/funds/fund/?id=${c.fundId}'">
  ...
  <div class="demo-grid demo-two">
    ...
    ${panel("Retirement position", `
      ...
      <div class="rp-note" style="margin-top:12px">
        ...
        <a href="/evolve/planning/?hh=${h.id}" style="color:var(--color-blue)">Run the full projection &rarr;</a>
      </div>`)}
  </div>`;
}
