...
function loadScenario() {
  ...
  try {
    const raw = JSON.parse(localStorage.getItem("ev-plan:" + plHh) || "null");
...
function saveScenario() {
  try { localStorage.setItem("ev-plan:" + plHh, JSON.stringify(plState)); } catch (e) {}
  render();
}
function plSet(k, v) { plState[k] = isNaN(+v) ? v : +v; saveScenario(); }
function plPick(id) { plHh = id; plState = loadScenario(); render(); }
function plReset() { try { localStorage.removeItem("ev-plan:" + plHh); } catch (e) {} plState = loadScenario(); render(); }
...
function runMonteCarlo() {
  ...
  const rnd = _rand("ev-mc-" + plHh + plState.spending + plState.longevity + plState.allocation + plState.inflation);
  const paths = 1000;
  let success = 0;
